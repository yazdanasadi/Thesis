#!/bin/bash
#SBATCH --job-name=mimic_fld_full
#SBATCH --output=logs/mimic_fld_full_%j.out
#SBATCH --error=logs/mimic_fld_full_%j.err
...
JSON_PATH="${1:-${CLUSTER_DIR}/mimic_FLD.json}"
LOG_ROOT="${2:-${REPO_ROOT}/runs/mimic_fld_full}"
...
for fn_name, meta in sorted(functions.items()):
    ...
    print(f"[mimic_fld] {run_dataset}/{fn_name} :: {display_cmd}")
    sys.stdout.flush()

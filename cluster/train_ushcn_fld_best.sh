#!/bin/bash
#SBATCH --job-name=ushcn_fld_full
#SBATCH --output=logs/ushcn_fld_full_%j.out
#SBATCH --error=logs/ushcn_fld_full_%j.err
...
JSON_PATH="${1:-${CLUSTER_DIR}/USHCN_FLD.json}"
LOG_ROOT="${2:-${REPO_ROOT}/runs/ushcn_fld_full}"
...
for fn_name, meta in sorted(functions.items()):
    ...
    print(f"[fld] {dataset}/{fn_name} :: {display_cmd}")
    sys.stdout.flush()

#!/bin/bash
#SBATCH --job-name=activity_fld_full
#SBATCH --output=logs/activity_fld_full_%j.out
#SBATCH --error=logs/activity_fld_full_%j.err
...
JSON_PATH="${1:-${CLUSTER_DIR}/activity_FLD.json}"
LOG_ROOT="${2:-${REPO_ROOT}/runs/activity_fld_full}"
...
for fn_name, meta in sorted(functions.items()):
    ...
    print(f"[activity_fld] {dataset}/{fn_name} :: {display_cmd}")
    sys.stdout.flush()

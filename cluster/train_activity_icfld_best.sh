#!/bin/bash
#SBATCH --job-name=activity_icfld_full
#SBATCH --output=logs/activity_icfld_full_%j.out
#SBATCH --error=logs/activity_icfld_full_%j.err
...
JSON_PATH="${1:-${CLUSTER_DIR}/activity_ICFLD.json}"
LOG_ROOT="${2:-${REPO_ROOT}/runs/activity_icfld_full}"
...
dataset = data.get("dataset", "activity")

train_script = repo_root / "FLD_ICC" / "train_FLD_ICC.py"
...
for fn_name, meta in sorted(functions.items()):
    ...
    def add_arg(flag: str, key, required: bool = False, bool_flag: bool = False):
    ...
    if embed_dim is None:
        raise ValueError(f"Could not determine embedding dim for function '{fn_name}'.")
    ...
    add_arg("--early-stop", ("early_stop", "early_stop_patience"))
    ...
    print(f"[activity_icfld] {dataset}/{fn_name} :: {display_cmd}")
    sys.stdout.flush()

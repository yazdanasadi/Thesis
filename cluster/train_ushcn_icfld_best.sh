#!/bin/bash
#SBATCH --job-name=ushcn_icfld_full
#SBATCH --output=logs/ushcn_icfld_full_%j.out
#SBATCH --error=logs/ushcn_icfld_full_%j.err
...
# Determine repo root (needs FLD & FLD_ICC directories)
REPO_ROOT=""
...
JSON_PATH="${1:-${CLUSTER_DIR}/USHCN_ICFLD.json}"
LOG_ROOT="${2:-${REPO_ROOT}/runs/ushcn_icfld_full}"
...
dataset = data.get("dataset", "USHCN")

train_script = repo_root / "FLD_ICC" / "train_FLD_ICC_ushcn.py"
...
for fn_name, meta in sorted(functions.items()):
    ...
    add_arg("-ld", "latent_dim", required=True)
    add_arg("-ed", "embed_dim", required=True)
    add_arg("-nh", "num_heads", required=True)
    add_arg("-dp", "depth", required=True)
    add_arg("-bs", "batch_size", required=True)
    add_arg("-e", "epochs", required=True)
    add_arg("-es", "early_stop", required=True)
    add_arg("-lr", ("lr", "learn_rate"), required=True)
    ...
    print(f"[icfld] {dataset}/{fn_name} :: {display_cmd}")
    sys.stdout.flush()

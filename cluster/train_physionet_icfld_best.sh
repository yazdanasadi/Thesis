#!/bin/bash
#SBATCH --job-name=physionet_icfld_full
#SBATCH --output=logs/physionet_icfld_full_%j.out
#SBATCH --error=logs/physionet_icfld_full_%j.err
...
JSON_PATH="${1:-${CLUSTER_DIR}/physionet_ICFLD.json}"
LOG_ROOT="${2:-${REPO_ROOT}/runs/physionet_icfld_full}"
...
for fn_name, meta in sorted(functions.items()):
    ...
    print(f"[physionet_icfld] {run_dataset}/{fn_name} :: {display_cmd}")
    sys.stdout.flush()

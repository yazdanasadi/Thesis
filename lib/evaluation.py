...
def evaluation_collect(model, dataloader, n_batches):
    model.eval()
    ys, yhats = [], []
    with torch.no_grad():
        it = 0
        for batch in dataloader:
            ...
            y_pred = out['yhat'].detach().cpu().numpy() if isinstance(out, dict) and 'yhat' in out \
                     else out.detach().cpu().numpy()
            ys.append(y_true); yhats.append(y_pred)
            it += 1
            if it >= n_batches: break
    y_all = np.concatenate(ys, axis=0)
    yhat_all = np.concatenate(yhats, axis=0)
    # Plain (placeholder) metrics in current space; caller will replace with TSDM metrics
    err = yhat_all - y_all
    mse = float(np.mean(err**2)); rmse = float(np.sqrt(mse))
    mae = float(np.mean(np.abs(err)))
    mape = float(np.mean(np.abs(err) / np.maximum(np.abs(y_all), 1e-12)) * 100.0)
    res = {"loss": mse, "mse": mse, "rmse": rmse, "mae": mae, "mape": mape}
    return res, y_all, yhat_all
